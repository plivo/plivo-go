...
func TestMultiPartyCallAbortTransfer_Post(t *testing.T) {
	expectResponse("MultiPartyCallResponse.json", 200)
	testPhloId := "PhloId"
	testNodeId := "NodeId"
	phloGet, _ := phloClient.Phlos.Get(testPhloId)
	...
	cl := phloClient.httpClient
	phloClient.httpClient = nil
	expectResponse("MultiPartyCallAbortTransferResponse.json", 200)

	_, err = multiPartyCallGet.AbortTransfer(payload)
	if err == nil {
		phloClient.httpClient = cl
		panic(errors.New("error expected"))
	}
